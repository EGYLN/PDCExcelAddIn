...
namespace BBS.ST.BHC.BSP.PDC.Lib
{
  ...
  {
    private static String MyKey = "34Po9D23";

    private static SymmetricAlgorithm   MySymmetricAlgorithm;
    private static SHA256               MySHA;
    ...
    static EncryptUtil()
    {
      MySymmetricAlgorithm = new RijndaelManaged();

      MySHA = SHA256.Create();


      MySymmetricAlgorithm.BlockSize = 256;

      MySymmetricAlgorithm.KeySize = 256;

      MySymmetricAlgorithm.Padding = PaddingMode.PKCS7;

      MySymmetricAlgorithm.IV = MySHA.ComputeHash(Encoding.UTF8.GetBytes("a3eCT675yX"));
    }
    ...
    public static String Decrypt(String text)
    {
      ...
      MySymmetricAlgorithm.Key = MySHA.ComputeHash(Encoding.UTF8.GetBytes(MyKey));
    ...
    private static Byte[] Decrypt(Byte[] input)
    {
      ...
      try
      {
        ...
        transform = MySymmetricAlgorithm.CreateDecryptor();
    ...
    private static Byte[] Encrypt(Byte[] input)
    {
      ...
      try
      {
        ...
        transform = MySymmetricAlgorithm.CreateEncryptor();
